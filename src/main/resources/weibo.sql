...
CREATE TABLE `comment` (
  ...
  `author_id` int(32) NOT NULL COMMENT '用户ID',
  PRIMARY KEY (`id`),
  KEY `reply-weibo` (`reply_weibo_id`),
  CONSTRAINT `reply-weibo` FOREIGN KEY (`reply_weibo_id`) REFERENCES `weibo` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

alter table comment add constraint fk_user_comments foreign key(author_id) references user(id);
...
CREATE TABLE `weibo` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `weibo_author` FOREIGN KEY (`author_id`) REFERENCES `user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
